...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Проверяем текущий статус задачи (опционально)
    // Сейчас проверяем при ЛЮБОМ создании/изменении задачи в форме
    ...
    const needCheck = statusLower.includes('выплат') ||
                      ...
                      statusLower.includes('работе') ||
                      data.trigger === 'manual' ||
                      data.event === 'task.created' ||
                      data.event === 'task.field_changed';
