...
export async function runBankCheck(taskId, { pyrusRequest, addCommentWithFieldUpdate }) {
  ...
  // Если уже проверено — выходим молча (чтобы webhook/cron не дублировали)
  if (fieldMap[FIELDS.CHECK_RESULT]) {
    console.log(`[CHECK] task=${taskId} already checked, skip`);
    return { ok: true, success: true, skipped: 'already checked' };
  }
  ...
  // 11. Тип учреждения (только инфо, без предупреждений)
  if (bank.opf?.full || bank.opf?.type) {
