...
export async function addCommentWithFieldUpdate(taskId, fieldUpdates, text) {
  ...
  console.log(`[PYRUS COMMENT] task=${taskId} status=${response.status} body=${t.substring(0, 300)}`);
  if (!response.ok) {
    throw new Error(`Pyrus comment failed: ${response.status} ${t.substring(0, 200)}`);
  }
  if (!t) return {};
  try {
    return JSON.parse(t);
  } catch (e) {
    throw new Error(`Parse error: ${t.substring(0, 200)}`);
  }
}
